...
private:
    ...
    int contarElementos(Nodo<T>* nodo) {
        if (nodo == nullptr) {
            return 0; // Si el nodo es null, no hay elementos
        }
        return 1 + contarElementos(nodo->izq) + contarElementos(nodo->der);
    }
