...
public:
    ...
    // Modificación para devolver punteros
    vector<Client<T>*> getAllClients() {
        vector<Client<T>*> clientPointers;
        for (auto& client : clients) {
            clientPointers.push_back(&client);  // Agregar punteros a los objetos clientes
        }
    ...
    Client<T>* findClientBus(string user) {
        for (auto& client : clients) {
            if (client.GetUser() == user) {  // Buscar solo por usuario
                return &client;
            }
        }
